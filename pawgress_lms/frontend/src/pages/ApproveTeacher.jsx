              ...
              <div className="col-span-full flex justify-center items-center min-h-64">
                <p className="text-cookie-darkbrown text-2xl">
                  ...
                  {activeTab === "approved" &&
                    ...
                    "There are no approved teachers"}
                  {activeTab === "banned" &&
                    bannedTeachers.length === 0 &&
                    "There are no banned teachers"}
                  {((activeTab === "pending" && pendingTeachers.length > 0) ||
                    (activeTab === "approved" && approvedTeachers.length > 0) ||
                    (activeTab === "banned" && bannedTeachers.length > 0)) &&
          ...
          <div className="flex flex-col gap-3 mt-6 px-4 w-full max-w-2xl">
            {filteredTeachers.length > 0 ? (
              filteredTeachers.map((teacher) => (
                <div
                  key={teacher.id}
                  className="bg-cookie-white rounded-lg shadow-lg p-4 flex items-center justify-between gap-4"
                >
                  <div>
                  ...
                  <div className="flex gap-2">
                    {activeTab === "pending" && (
                      <>
                        <button onClick={() => handleApprove(teacher.id)}>
                          <Check className="size-8 p-1 rounded-full bg-green-100 text-green-600 hover:bg-green-200 transition-colors duration-200" />
                        </button>
                        <button onClick={() => handleDisapprove(teacher.id)}>
                          <X className="size-8 p-1 rounded-full bg-red-100 text-red-600 hover:bg-red-200 transition-colors duration-200" />
                        </button>
                      </>
                    )}
                    {activeTab === "approved" && (
                      <button
                        onClick={() => handleBan(teacher.id)}
                        className="px-3 py-1 text-sm font-semibold text-red-600 bg-red-100 rounded-full hover:bg-red-200 transition-colors duration-200"
                      >
                        Ban
                      </button>
                    )}
                    {activeTab === "banned" && (
                      <button
                        onClick={() => handleUnban(teacher.id)}
                        className="px-3 py-1 text-sm font-semibold text-green-600 bg-green-100 rounded-full hover:bg-green-200 transition-colors duration-200"
                      >
                        Unban
                      </button>
                    )}
